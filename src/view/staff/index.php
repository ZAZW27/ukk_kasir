...
<div class="w-full flex justify-center items-center py-12">
    <div class="">
        <div class="row row--top-40">
            <div class="col-md-12 flex gap-4">
                ...
                <button id="tambah-pegawai-btn" class="btn bg-blue-500 px-4 py-1 rounded-md shadow-md shadow-blue-300/70 text-white font-semibold">Tambah</button>
            </div>
    ...
    <!-- jangan lupa opacitynya dikasih 0 sama pointer-events-none lagi yach -->
    <div id="modal-bg" class="w-full h-full bg-slate-700/40 fixed pointer-events-none opacity-0 top-0 left-0 flex justify-center items-center transition-all duration-150 ease-linear">
        <!-- FORM UNTUK INSERT PEGAWAI BARU  -->
        <form id="modal-tambah-pegawai"  action="crud/aksi-tambah.php" method="post" class="bg-white flex flex-col justify-center scale-0 hidden items-center p-4 w-full md:w-[35rem] rounded-lg shadow-xl shadow-slate-50/50 transition-all duration-200 ease-out">
            ...
            <div class="flex flex-col relative z-10 w-full">
                <div class="input-form flex flex-col mt-1 w-[90%] ">
                    ...
                    <input name="nama-pegawai" type="text" class="border-b-2 border-slate-700 focus:right-0 focus:outline-none bg-transparent">
                </div>
                <div class="input-form flex flex-col mt-1 w-[90%]">
                    <label for="">Username</label>
                    <input name="username" type="text" class="border-b-2 border-slate-700 focus:right-0 focus:outline-none bg-transparent">
                </div>
                <div class="input-form flex flex-col mt-1 w-[90%]">
                    <label for="">Password</label>
                    <input name="password" type="password" class="border-b-2 border-slate-700 focus:right-0 focus:outline-none bg-transparent">
                </div>
                <div class="input-form flex flex-col mt-1 w-[90%]">
                    <label for="">Bidang</label>
                    <select name="bidang" id="" class="border-b-2 border-slate-700 focus:right-0 focus:outline-none bg-transparent">
                ...
                <div class=" flex justify-around items-center w-full mt-4">
                    <button type="button" id="hapus-modal-tambah" class="btn bg-red-500 rounded-md shadow-md shadow-red-300/20 w-20 text-white font-semibold py-1 px-2">Go back!</button>
        ...
        <!-- FORM UNUTK UPDATE PEGAWAI -->
        <form id="modal-update-pegawai"  action="crud/aksi-update.php" method="post" class="bg-white flex flex-col justify-center scale-0 hidden items-center p-4 w-full md:w-[35rem] rounded-lg shadow-xl shadow-slate-50/50 transition-all duration-200 ease-out">
        </form>
    </div>
</div>
<script>
        ...
        $(document).off('click', '#update-modal').on('click', '#update-modal', function(){
            ...
            $.ajax({
                ...
                success: function(msg){
                    ...
                    setTimeout(() => {
                        $('#modal-update-pegawai').removeClass('scale-0 hidden').addClass('scale-110')
                        setTimeout(() => {
                            $('#modal-update-pegawai').removeClass('scale-110').addClass('scale-100')
                        }, 200);
                    }, 50);
                    $('#modal-update-pegawai').html(msg);
                }
            })
        })
    })
